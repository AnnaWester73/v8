# User story 1
Som en användare, vill jag kunna skapa en timer-widget,
så att jag kan mäta tid

# User story 2
Som en användare, vill jag kunna lägga till en anteckning till timer-widget,
så att jag kan visa information kopplad till tiden.

# User story 3
Som en användare, vill jag kunna ta bort en timer-widget,
så att jag inte behöver vänta till tiden gått ut.

# User story 4
Som en användare, vill jag kunna ta bort en anteckning till en timer-widget,
så att jag inte visar någon anteckning i en timer-widget

# User story 5
Som en användare, vill jag kunna skapa flera timer-widget,
så att jag kan mäta fler tider samtidigt

# User story 6
Som en användare, vill jag kunna ändra tiden på timer-widget,
så att den anpassar sig till att mäta korrekt tid.

# User Story 7
Som en användare, vill jag kunna ha funktioner starta, stopp och pausa en timer widget,
så att jag kan kontrollera timer efter behov.

# User story 8
Som en användare, vill jag kunna ändra text på anteckningar,
så att jag kan ändra text efter behov.

# User story 9
Som en användare, vill jag kunna ändra färger på widget,
så att jag kan få olika teman på widgets.

# User story 10
Som en användare, vill jag kunna byta plats på timer-widgets,
så att jag kan anpassa vart jag vill att det ska visas.




